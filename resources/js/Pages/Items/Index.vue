        ...
        <table class="shadow-lg w-full border mt-4 ml-2 rounded-xl">
          <thead>
            <tr class="bg-indigo-100">
              ...
              <th class="py-2 px-4 border">UnitType</th>
          ...
          <tbody>
            <tr v-for="item in data" :key="item.id">
              ...
              <td class="py-1 px-4 border">{{ item.hscode }}</td>
              <td class="py-1 px-4 border">{{ item.unit_id }}</td>
...
export default {
  ...
  methods: {
    ...
    edit(id) {
      this.$inertia.get(route("items.edit", id));
    },
