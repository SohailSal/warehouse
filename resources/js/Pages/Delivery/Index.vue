        ...
        <table class="shadow-lg w-full border mt-4 ml-2 rounded-xl">
          <thead>
			...
			c0,1.105,0.896,2,2,2h27.135c1.104,0,2-0.895,2-2v-3.842C89.255,90.732,88.361,89.838,87.255,89.838z"
              ...
              <th class="py-2 px-4 border">Cash No</th>
              <th class="py-2 px-4 border">Vehicle No</th>
              <th class="py-2 px-4 border">Item Name</th>
              <th class="py-2 px-4 border">Qunatity</th>
              <th class="py-2 px-4 border">Action</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="item in balances.data" :key="item.id">
              <td class="py-1 px-4 border">{{ item.file_id }}</td>
              <td class="py-1 px-4 border">{{ item.date }}</td>
              <td class="py-1 px-4 border">{{ item.cash_no }}</td>
              <td class="py-1 px-4 border">{{ item.vehicle_no }}</td>
              <td class="py-1 px-4 border">{{ item.item_id }}</td>
  ...
  watch: {
    params: {
      handler: throttle(function () {
        ...
        this.$inertia.get(this.route("deliveries"), params, {
          replace: true,
          preserveState: true,
        });
      }, 150),
